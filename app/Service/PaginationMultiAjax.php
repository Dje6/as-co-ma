...
class PaginationMultiAjax extends CustomModel
{
  ...
  public function pagination($page,$nb_page,$route,$arg='')
  {
    ...
    if($page == $nb_page && $page != 1){
      ...
      $html .='<a href="'.CustomController::generateUrl($route,$argumentFull).'" class="ajaxPagin '.($page-1).'"> << </a>';
  ...
  public function liste($nb_page,$route,$page,$argumentFull)
  {
    ...
    for($i=1; $i <= $nb_page; $i++) {
      $argumentFull['page'] = $i;
      if($i == $page){
        $style = '<span class="actuel"><a href="'.CustomController::generateUrl($route,$argumentFull).'" class="ajaxPagin '.$i.'">'.$i.'</a></span>';
      }elseif($i > $page) {
        $style = '<span class="voisin"><a href="'.CustomController::generateUrl($route,$argumentFull).'" class="ajaxPagin '.$i.'">'.$i.'</a></span>';
      }elseif($i < $page){
        $style = '<span class="voisin"><a href="'.CustomController::generateUrl($route,$argumentFull).'" class="ajaxPagin '.$i.'">'.$i.'</a></span>';
      }
